...
class MyVideo extends StatefulWidget {
  ...
  const MyVideo({Key? key, required this.player}) : super(key: key);

  @override
  State<MyVideo> createState() => _MyVideoState();
}

class _MyVideoState extends State<MyVideo> with AutomaticKeepAliveClientMixin {
  ...
  StreamSubscription<VideoDimensions>? _videoDimensionsSubscription;
  double? _videoWidth;
  double? _videoHeight;
  ...
  void initState() {
    ...
    _videoDimensionsSubscription = widget.player.videoDimensionsStream.listen((dimensions) {
...
class ControlState extends State<Control> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    playPauseStream =
        player.playbackStream.listen((event) => setPlaybackMode(isPlaying: event.isPlaying));
                ...
                children: [
                            ...
                            IconButton(
                              ...
                              onPressed: () {
                                int positionInMilliseconds =
                                    player.position.position?.inMilliseconds ?? 0;
                                if (!(positionInMilliseconds - 10000).isNegative) {
                                  positionInMilliseconds -= 10000;
                                }
                                player.seek(Duration(milliseconds: positionInMilliseconds));
                                setState(() {});
                              },
                            ),
                            ...
                            IconButton(
                              ...
                              onPressed: () {
                                final int durationInMilliseconds =
                                    player.position.duration?.inMilliseconds ?? 0;
                                int positionInMilliseconds =
                                    player.position.position?.inMilliseconds ?? 1;
                                if ((positionInMilliseconds + 10000) <= durationInMilliseconds) {
                                  positionInMilliseconds += 10000;
                                  player.seek(Duration(milliseconds: positionInMilliseconds));
                                  setState(() {});
                                }
                  ...
                  if (widget.showFullscreenButton)
                    Positioned(
                      ...
                      child: IconButton(
                        onPressed:
                            !widget.isFullscreen ? widget.enterFullscreen : widget.exitFullscreen,
                        iconSize: 24,
                        icon: Icon(
